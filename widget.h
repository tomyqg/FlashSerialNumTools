#ifndef WIDGET_H
#define WIDGET_H

#include <QWidget>
#include "ui_widget.h"

#include <QNetworkInterface>
#include <QSettings>
#include "mytcpclient.h"
#include <QProgressDialog>
#include <QMessageBox>
#include <QKeyEvent>
#include <QDateTime>
#include <QFileDialog>
#include <QMenu>
#include <QAction>
#include <QTimer>


#define TCPSERVER 10
#define TCPCLIENT 20
#define UDPSERVER 30

/* Table of CRC values for high-order byte */
static const uint8_t table_crc_hi[] = {
    
};

/* Table of CRC values for low-order byte */
static const uint8_t table_crc_lo[] = {
    
};

class MyProgressDlg;

QT_BEGIN_NAMESPACE
namespace Ui { class Widget; }
QT_END_NAMESPACE

class Widget : public QWidget
{
    Q_OBJECT

public:
#define    WIDGET_WIDTH    790
#define    WIDGET_HEIGTH_MAIN   190
#define    WIDGET_HEIGTH_EXT    380

#define    REC_BUFF_MAX_LEN           (1024)
#define    FC_QUERY       0x04  // 功能码-查询
#define    FC_CTL         0x03  // 功能码-控制
#define    CC_QUERY_CONN  0x00  // 命令字-查询设备是否在线指令
#define    CC_SET_IAP     0x03  // 命令字-在线升级指令

#define    CC_FLASH_SERIAL_NUM  0x04  // 烧录生产序号
#define    CC_FLASH_CODE        0x05  // 烧录产品编码

#define    OFFSET_FC      8    // 功能码在数据包中的索引
#define    OFFSET_CC      7    // 命令字在数据包中的索引
#define    OFFSET_RESULT  15   // 返回的结果数据在数据包中的索引（帧头4，长度2，中间7，数据域长度2）

#define    SUSPEND_TIME      100   // 100ms查询一次结果
#define    WAITING_TIMEOUT 5000  // 5s超时

#define    WIFI_SIZE_FRAME_HEADER      4
#define    WIFI_SIZE_FRAME_LEN         2

    typedef enum {
        step_chk_conn,
        step_flash_serial_num,
        step_flash_product_code,
    }E_FlashStep;
    typedef enum{
        none,
        dev_conn_success,
        flash_serial_num_success,
        flash_serial_num_failed,
        flash_product_code_success,
        flash_product_code_failed
    }E_FlashResult;

public:
    Widget(QWidget *parent = nullptr);
    ~Widget();

public:
    void loadSettings();
    void saveSettings();
    bool setupConnection(quint8 type);
    void flash_chk_dev_conn();
    void flash_serial_num();
    void flash_product_code();

private slots:
    void onTcpClientButtonClicked();
    void onTcpClientNewConnection(const QString &from, quint16 port);
    void onTcpClientTimeOut();
    void onTcpClientStopButtonClicked();
    void onTcpClientDisconnectButtonClicked();
    void onTcpClientDisconnected();
    void onTcpClientAppendMessage(const QString &from, const QByteArray &message);
    void onDeviceConnSuccess();
    void onDeviceDisconn();

    void onTcpClientConnectFailed();

private slots:
    void on_btn_showDetailInfo_toggled(bool checked);
    void onClearDetailInfo();
    void on_textBrowser_DetailInfo_customContextMenuRequested(const QPoint &pos);

    void on_lineEdit_ProductSerialNum_textEdited(const QString &arg1);

    void on_lineEdit_ProductCode_textEdited(const QString &arg1);

    QString formatInput(QString &str, uint32_t width_limit);

private:
    void logMsg(QString msg);

private:
    Ui::Widget *ui;

    MyTCPClient *mytcpclient = nullptr;
    QString settingsFileDir;
    QHostAddress tcpClientTargetAddr;
    quint16 tcpClientTargetPort;

    bool devOnlineStatus;

    E_FlashResult m_result;

    MyProgressDlg *connPDlg = nullptr;
    QFileDialog *fileDlg = nullptr;

    quint8 rec_buff[REC_BUFF_MAX_LEN] = {0};

    QTimer *tcpConnectTimer = nullptr;
};

class MyProgressDlg : public QProgressDialog
{
    Q_OBJECT

public:
    explicit MyProgressDlg(QWidget *parent = nullptr);
    ~MyProgressDlg();

    void keyPressEvent(QKeyEvent *event);

//    QTimer *progressTimer = nullptr;
    quint32 progressCnt = 0;
};

#endif // WIDGET_H
